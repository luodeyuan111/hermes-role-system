...
# /api/library/preview and /api/library/thumb are here because browser
# <img>/<iframe> tags can't set the session header either.
    ...
    try:
        ...
        for child in target.iterdir():
            ...
            try:
                ...
                # Skip entries that cannot be resolved/statted (e.g. a
                # dangling symlink like Steam's ~/.steampath) instead of
                # failing the entire directory listing with a 500.
                continue
    except PermissionError:
...
class ManagedFileOpen(BaseModel):
    path: str
    """When true, open the containing folder instead of the file itself
    ("在文件夹中显示")."""
    reveal: bool = False


@app.post("/api/files/open")
async def open_managed_file(payload: ManagedFileOpen, request: Request):
    """Open a managed path with the host's default application (xdg-open).

    The dashboard usually runs on the same machine as the files, so "在系统
    里打开" should launch the OS handler directly instead of pushing another
    browser download at the user. ``reveal`` opens the containing folder
    (file-manager management: rename/move/edit there). Best-effort and
    fire-and-forget: xdg-open is spawned detached, so a missing handler on
    the host surfaces as "nothing happens", not a hung request.
    """
    policy, target, _display_path = _resolve_managed_path(payload.path, request)
    if payload.reveal and not target.is_dir():
    ...
    args = [opener, str(target)] if opener else None
    if args is None:
        gio = shutil.which("gio")
        args = [gio, "open", str(target)] if gio else None
    if args is None:
        raise HTTPException(status_code=503, detail="No xdg-open/gio available on the host")
    try:
        subprocess.Popen(
            args,
        ...
        raise HTTPException(status_code=500, detail=f"Could not launch opener: {exc}")
    return {"ok": True, **_managed_response_meta(policy)}
...
# ---------------------------------------------------------------------------
# 资料馆 (Library) — read-only file index over enrolled source trees.
#
# Source files are never written to; conversion products live in the library
# home and are bound to sources via a SQLite table (see hermes_cli.library).
# Every path param is resolved and must land inside an enabled enrolled root,
# otherwise 403 — same confinement idea as _resolve_managed_path above.
# /api/library/preview and /api/library/thumb also accept ?token= auth (see
# _QUERY_TOKEN_API_PATHS) so browser <img>/<iframe> tags can load them.
# ---------------------------------------------------------------------------

from hermes_cli import library as _library
...
@app.post("/api/library/feed/status")
...
@app.post("/api/library/feed/note")
...
@app.post("/api/library/note/save")
...
@app.get("/api/library/note/links")
...
@app.post("/api/library/file/delete")
...
@app.post("/api/library/file/paste")
...
@app.post("/api/library/file/forward")
