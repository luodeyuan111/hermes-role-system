...
export const api = {
  ...
  getSessions: (
    ...
    /** Source scoping: `source` 只看该类，`excludeSources` 排除该类（如 "cron"）。 */
    sourceScope?: { source?: string; excludeSources?: string },
  ) => {
    const scopeQs = sourceScope?.source
      ? `&source=${encodeURIComponent(sourceScope.source)}`
      : sourceScope?.excludeSources
        ? `&exclude_sources=${encodeURIComponent(sourceScope.excludeSources)}`
        : "";
    return fetchJSON<PaginatedSessions>(
      appendProfileParam(
        `/api/sessions?limit=${limit}&offset=${offset}&order=${order}${scopeQs}`,
        profile,
      ),
    );
  },
