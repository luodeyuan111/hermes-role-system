...
/**
 * Pull every file out of a DataTransfer (clipboard or drop), any MIME type.
 * Pasting a non-image file (PDF, docx, …) must become a file attachment
 * instead of falling through to a default text paste — the fallback text is
 * often a bare absolute path, which the chat then misreads as a slash
 * command and the turn goes nowhere.
 */
